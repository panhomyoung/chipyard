...
module MemAddrCalcUnit(
  ...
  wire [63:0] _GEN = io_req_bits_rs1_data[63:0] + {{52{io_req_bits_uop_imm_packed[19]}}, io_req_bits_uop_imm_packed[19:8]};	// @[functional-unit.scala:483:{42,70}]
  wire        misaligned = io_req_bits_uop_mem_size == 2'h1 & _GEN[0] | io_req_bits_uop_mem_size == 2'h2 & (|(_GEN[1:0])) | (&io_req_bits_uop_mem_size) & (|(_GEN[2:0]));	// @[functional-unit.scala:483:42, :508:{11,19,40}, :509:{11,19,40,46,56}, :510:{11,19,40,46}]
  wire        ma_ld = io_req_valid & io_req_bits_uop_uopc == 7'h1 & misaligned;	// @[functional-unit.scala:501:76, :509:56, :520:{53,63}]
  wire        ma_st = io_req_valid & (io_req_bits_uop_uopc == 7'h2 | io_req_bits_uop_uopc == 7'h43) & misaligned;	// @[functional-unit.scala:502:41, :509:56, :521:{54,65,89,104}]
  `ifndef SYNTHESIS	// @[functional-unit.scala:494:12]
    wire _T_9 = io_req_valid & io_req_bits_uop_ctrl_is_std;	// @[functional-unit.scala:494:28]
    always @(posedge clock) begin	// @[functional-unit.scala:494:12]
      if (~reset & _T_9 & io_req_bits_rs2_data[64]) begin	// @[functional-unit.scala:494:{12,28}, :495:24]
        if (`ASSERT_VERBOSE_COND_)	// @[functional-unit.scala:494:12]
          $error("Assertion failed: 65th bit set in MemAddrCalcUnit.\n    at functional-unit.scala:494 assert (!(io.req.valid && io.req.bits.uop.ctrl.is_std &&\n");	// @[functional-unit.scala:494:12]
        if (`STOP_COND_)	// @[functional-unit.scala:494:12]
          $fatal;	// @[functional-unit.scala:494:12]
      end
      if (~reset & _T_9 & io_req_bits_uop_fp_val) begin	// @[functional-unit.scala:494:28, :497:12]
      ...
      if (~reset & io_req_bits_uop_fp_val & io_req_valid & io_req_bits_uop_uopc != 7'h1 & io_req_bits_uop_uopc != 7'h2) begin	// @[functional-unit.scala:501:{10,76}, :502:41]
        if (`ASSERT_VERBOSE_COND_)	// @[functional-unit.scala:501:10]
          $error("Assertion failed: [maddrcalc] assert we never get store data in here.\n    at functional-unit.scala:501 assert (!(io.req.bits.uop.fp_val && io.req.valid && io.req.bits.uop.uopc =/=\n");	// @[functional-unit.scala:501:10]
        if (`STOP_COND_)	// @[functional-unit.scala:501:10]
          $fatal;	// @[functional-unit.scala:501:10]
      end
      if (~reset & ma_ld & ma_st) begin	// @[functional-unit.scala:520:63, :521:104, :537:10]
        if (`ASSERT_VERBOSE_COND_)	// @[functional-unit.scala:537:10]
          $error("Assertion failed: Mutually-exclusive exceptions are firing.\n    at functional-unit.scala:537 assert (!(ma_ld && ma_st), \"Mutually-exclusive exceptions are firing.\")\n");	// @[functional-unit.scala:537:10]
        if (`STOP_COND_)	// @[functional-unit.scala:537:10]
          $fatal;	// @[functional-unit.scala:537:10]
      end
    end // always @(posedge)
  `endif // not def SYNTHESIS
  assign io_resp_valid = io_req_valid & (io_brupdate_b1_mispredict_mask & io_req_bits_uop_br_mask) == 16'h0;	// @[functional-unit.scala:266:38, util.scala:118:{51,59}]
  ...
  assign io_resp_bits_uop_br_mask = io_req_bits_uop_br_mask & ~io_brupdate_b1_resolve_mask;	// @[util.scala:85:{25,27}]
  ...
  assign io_resp_bits_addr = {_GEN[38] ? (&(_GEN[63:39])) : (|(_GEN[63:39])), _GEN[38:0]};	// @[Cat.scala:33:92, functional-unit.scala:483:42, :484:{20,24,43,58}, :485:58, :486:43]
  assign io_resp_bits_mxcpt_valid = ma_ld | ma_st;	// @[functional-unit.scala:520:63, :521:104, :528:26]
  assign io_resp_bits_sfence_valid = io_req_valid & io_req_bits_uop_mem_cmd == 5'h14;	// @[functional-unit.scala:539:{45,72}]
  assign io_resp_bits_sfence_bits_rs1 = io_req_bits_uop_mem_size[0];	// @[functional-unit.scala:540:59]
  assign io_resp_bits_sfence_bits_rs2 = io_req_bits_uop_mem_size[1];	// @[functional-unit.scala:541:59]
  assign io_resp_bits_sfence_bits_addr = io_req_bits_rs1_data[38:0];	// @[functional-unit.scala:542:33]
endmodule
