...
class Genus(HammerSynthesisTool, CadenceTool):
    ...
    def fill_outputs(self) -> bool:
        ...
        if self.ran_write_outputs:
            ...
            if self.get_setting('synthesis.genus.reduce_drive_usage'):
                return True
    ...
    def do_between_steps(self, prev: HammerToolStep, next: HammerToolStep) -> bool:
        ...
        if self.get_setting('synthesis.genus.no_write_db'):
            return True
        # Write a checkpoint to disk.
        self.append("write_db -to_file pre_{step}".format(step=next.name))
        return True
    ...
    def init_environment(self) -> bool:
        ...
        self.logger.info(f"Generating MMMC script content: {self.generate_mmmc_script()}") # 添加日志
        self.write_contents_to_path(self.generate_mmmc_script(), mmmc_path)
        self.logger.info(f"MMMC TCL script written to: {mmmc_path}") # 添加日志
    ...
    def syn_map(self) -> bool:
        ...
        if self.hierarchical_mode not in [HierarchicalMode.Flat, HierarchicalMode.Top]:
            self.append("update_names -module -log hier_updated_names.log -suffix _{MODULE}".format(MODULE=self.top_module))
        return True
    ...
    def add_tieoffs(self) -> bool:
        ...
        self.append("set_db message:WSDF-201 .max_print 20")
        self.append("set_db use_tiehilo_for_const duplicate")
        ...
        if any(cnt>1 for cnt in corner_counts.values()):
            self.append("set ACTIVE_VIEW [string map { .setup_view {} .hold_view {} .extra_view {} } [get_db analysis_view:[get_analysis_views] .name]]")
            self.append("set HI_TIEOFF [get_db base_cell:{TIE_HI_CELL} .lib_cells -if {{ .library.default_opcond == $ACTIVE_VIEW }}]".format(TIE_HI_CELL=tie_hi_cell))
            self.append("set LO_TIEOFF [get_db base_cell:{TIE_LO_CELL} .lib_cells -if {{ .library.default_opcond == $ACTIVE_VIEW }}]".format(TIE_LO_CELL=tie_lo_cell))
            self.append("add_tieoffs -high $HI_TIEOFF -low $LO_TIEOFF -max_fanout 1 -verbose")
        else:
            self.append("add_tieoffs -high {HI_TIEOFF} -low {LO_TIEOFF} -max_fanout 1 -verbose".format(HI_TIEOFF=tie_hi_cell, LO_TIEOFF=tie_lo_cell))
        return True

    def generate_reports(self) -> bool:
        """Generate reports."""
        # TODO: extend report generation capabilities
        self.append("write_reports -directory reports -tag final")
        return True
    ...
    def write_outputs(self) -> bool:
        ...
        if not self.get_setting('synthesis.genus.reduce_drive_usage'):
    ...
    def run_genus(self) -> bool:
        ...
        if self.get_setting('synthesis.genus.silent_run'):
